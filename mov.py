...
class timeSVDpp:
 ...
 #           print ('updates for the user:', userId, ' STARTED...')
            ...
            tmpe_SumYj = np.zeros(self.factors, dtype='float')
            ...
            if sz > 0:
                ...
                for it in range(sz):
                    ...
                    #self.bu_t[userId][timestamp_]  += 0.01 * (error - 0.01 * self.bu_t[userId][timestamp_]) 
#                    self.bu_t[userId][timestamp_]  += self.gamma_1 * (error -  self.tau_6 * self.bu_t[userId][timestamp_])
#                    self.bi_bin[itemid][self.calBin(timestamp_)] += 0.01 * (error - 0.01* self.bi_bin[itemid][self.calBin(timestamp_)]) # 
                    self.bi_bin[itemid][self.calBin(timestamp_)] += self.gamma_1 * (error - self.tau_6* self.bi_bin[itemid][self.calBin(timestamp_)])
#                    self.alpha_u[userId] += 0.01 * (error * self.dev(userId, timestamp_) - 0.01 * self.alpha_u[userId]) # 
#                    self.alpha_u[userId] += self.g_alpha * (error * self.dev(userId, timestamp_) - self.l_alpha * self.alpha_u[userId])
   ...
   #                     self.itemFactors[itemid][k] += 0.01 * (error * (u_f + sqrtNum * self.sumMW[userId][k]) - 0.01 * i_f) # 
                        self.itemFactors[itemid][k] += self.gamma_1 * (error * (u_f + sqrtNum * self.sumMW[userId][k] + self.alpha_u_k[userId][k]*self.dev(userId,timestamp_)) - 0.015 * i_f)
   #                     self.alpha_u_k[userId][k] += 0.01 * (error * self.dev(userId, timestamp_) - 0.01 * self.alpha_u_k[userId][k]) # 
                        self.alpha_u_k[userId][k] += self.g_alpha * (error * self.dev(userId, timestamp_) * i_f - self.l_alpha * self.alpha_u_k[userId][k])
   #                     self.userFactors_t[userId][k][timestamp_] += 0.01 * (error * i_f - 0.01 * u_f_t) # 
                        self.userFactors_t[userId][k][timestamp_] += self.gamma_1 * (error * i_f - 0.015 * u_f_t)
                        tmpe_SumYj[k] += error * sqrtNum * i_f
  ...
  #                      self.y_j[itID][f] += 0.01 * (tmpSum[f] - 0.01 * tmpMW) # 
                        self.y_j[itID][f] += self.gamma_1 * (tmpe_SumYj[f] - 0.015 * tmpMW)
    ...
    def calBin(self, day_of_rating):
        ...
        Bin_index = np.minimum(self.nBins - 1, int((day_of_rating - self.min_time)/interval))
    ...
    def prediction(self, u, i, day_ind):
        ...
        tmp = 0
        for k in range(self.factors):
            tmp  += ((self.userFactors[u][k] + self.alpha_u_k[u][k] * self.dev(u, day_ind) + self.userFactors_t[u][k][day_ind]) + (sqrtNum * self.sumYj[u][k])) * self.itemFactors[i][k]
        prediction = self.average + self.bu[u] + self.bi[i] + self.bi_bin[i][self.calBin(day_ind)] 
        return prediction

    # evaluating the model using RMSE
    def RMSE(self):
        with open("C:/Users/hp/Desktop/test/u1test.test", 'r',encoding='ISO-8859-1') as f:
...
timesvd_pp = timeSVDpp(30, nFactors, nUsers, nItems, userItems, nBins, nDays, minTimestamp)
