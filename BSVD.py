...
class BSVD:
    ...
    def Prediction(self,User_id,Item_id):  
        ...
        self.Dic_u.setdefault(User_id,[])
        Rating=float(np.sum(self.q_i[Item_id]*(self.p_u[User_id])))
        # 对于高于5分或者低于1分的，统一处理
        if Rating>5:
            Rating=5
        if Rating<1:
    ...
    def Training(self,steps=200000,Gamma=0.005):
        for step in range(steps):
            ...
            for i in range(self.matrix.shape[0]):
                temp=Rand_k[i]
                User_id=self.matrix[temp,0]
                Item_id=self.matrix[temp,1]
                Rating=self.matrix[temp,2]
                predict=self.Prediction(User_id, Item_id)
                E_ui=Rating-predict
                RMSE+=E_ui**2
                MAE=MAE+abs(E_ui)
                #迭代
                self.p_u[User_id]+=Gamma*(E_ui*self.q_i[Item_id])
                self.q_i[Item_id]+=Gamma*(E_ui*self.p_u[User_id])                      
            Gamma=0.93*Gamma
